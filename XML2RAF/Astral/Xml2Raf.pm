...
# 2020-07-07 jmc v1.07  Bumped RAF version number to 0.03
# 2022-12-15 jmc v1.08  Ignore residue ids longer than 4 characters
...
#warn "$chain lastRes not defined\n" unless defined $rc->{lastRes};
	...
	# COMMENTED THIS OUT; no longer needed with numbers now available
	...
	# real output
	my $raf_line = "$pdbid$chain 0.03 38 $date 11101"
		     . ($rc->{notOneToOne} ? '0' : '1')
...
#    $in{$el} = 0;
	...
	# skip residues with id longer than 4 chars (5 including ins code)
	if ((defined $residue->{resID}) &&
	    (length($residue->{resID}) > 5)) {
	    warn "Long residue ID; skipping $chain, residue ".$residue->{resID}."\n";
	    return;
	}
